...
if [ -f /etc/secrets/robotkey.pem ]; then
    sudo cp /etc/secrets/robotkey.pem /data/srv/current/auth/acdcserver/dmwm-service-key.pem
    sudo cp /etc/secrets/robotcert.pem /data/srv/current/auth/acdcserver/dmwm-service-cert.pem
fi

# overwrite proxy if it is present in /etc/proxy
if [ -f /etc/proxy/proxy ]; then
    mkdir -p /data/srv/state/acdcserver/proxy
    ln -s /etc/proxy/proxy /data/srv/state/acdcserver/proxy/proxy.cert
...
# we need to populate acdcserver dbs into couch first
if [ -f /data/srv/state/couchdb/stagingarea/acdcserver ]; then
    ...
    source /data/srv/state/couchdb/stagingarea/acdcserver
fi

# use service configuration files from /etc/secrets if they are present
cdir=/data/srv/current/config/acdcserver
files=`ls $cdir`
...
# start the service
/data/srv/current/config/acdcserver/manage start 'I did read documentation'
