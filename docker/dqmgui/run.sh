...
if [ -f /etc/secrets/robotkey.pem ]; then
    sudo cp /etc/secrets/robotkey.pem /data/srv/current/auth/dqmgui/dmwm-service-key.pem
    sudo cp /etc/secrets/robotcert.pem /data/srv/current/auth/dqmgui/dmwm-service-cert.pem
fi

# overwrite proxy file with one from secrets
if [ -f /etc/secrets/proxy ]; then
    mkdir -p /data/srv/state/dqmgui/proxy
    /bin/cp -f /etc/secrets/proxy /data/srv/state/dqmgui/proxy/proxy.cert
fi
...
# we need to populate dqmgui dbs into couch first
if [ -f /data/srv/state/couchdb/stagingarea/dqmgui ]; then
