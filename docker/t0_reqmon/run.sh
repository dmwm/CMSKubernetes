...
if [ -f /etc/secrets/robotkey.pem ]; then
    sudo cp /etc/secrets/robotkey.pem /data/srv/current/auth/t0_reqmon/dmwm-service-key.pem
    sudo cp /etc/secrets/robotcert.pem /data/srv/current/auth/t0_reqmon/dmwm-service-cert.pem
fi

# overwrite proxy file with one from secrets
if [ -f /etc/secrets/proxy ]; then
    mkdir -p /data/srv/state/t0_reqmon/proxy
    /bin/cp -f /etc/secrets/proxy /data/srv/state/t0_reqmon/proxy/proxy.cert
fi
...
# we need to populate t0_reqmon dbs into couch first
if [ -f /data/srv/state/couchdb/stagingarea/t0_reqmon ]; then
