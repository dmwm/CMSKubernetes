...
if [ -f /etc/secrets/robotkey.pem ]; then
    sudo cp /etc/secrets/robotkey.pem /data/srv/current/auth/phedex/dmwm-service-key.pem
    sudo cp /etc/secrets/robotcert.pem /data/srv/current/auth/phedex/dmwm-service-cert.pem
fi

# overwrite proxy if it is present in /etc/proxy
if [ -f /etc/proxy/proxy ]; then
    mkdir -p /data/srv/state/proxy/proxy
    ln -s /etc/proxy/proxy /data/srv/state/proxy/proxy/proxy.cert
...
cdir=/data/srv/current/config/phedex
files=`ls $cdir`
...
# start the service
/data/srv/current/config/phedex/manage start 'I did read documentation'
