...
voms-proxy-init2 -rfc -valid 96:00 -cert /opt/rucio/certs/usercert.pem -key /opt/rucio/certs/new_userkey.pem -out /opt/rucio/certs/x509up -voms cms:/cms/Role=production -rfc -timeout 5
...
kubectl create secret generic  cms-analysisd-testbed-rucio-x509up --from-file=/opt/rucio/certs/x509up --dry-run -o yaml | kubectl apply --validate=false  -f  -
