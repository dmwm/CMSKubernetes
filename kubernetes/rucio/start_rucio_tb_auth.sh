...
docker kill cms_rucio_tb_auth
docker rm cms_rucio_tb_auth

docker run -d --privileged  \
  --name cms_rucio_tb_auth \
...
export RUCIO_CFG_DATABASE_DEFAULT=`cat /afs/cern.ch/user/e/ewv/CMSKubernetes/kubernetes/rucio/dev_db.txt`

docker kill cms_rucio_dev_auth
docker rm cms_rucio_dev_auth

docker run -d --privileged  \
  --name cms_rucio_dev_auth \
  -e RUCIO_CFG_DATABASE_DEFAULT \
  -e RUCIO_DEFINE_ALIASES \
  -e RUCIO_LOG_LEVEL \
  -e RUCIO_ENABLE_SSL \
